...
COMMON W_base,base_plot
COMMON GRAPH,spectra,stoks,wave,dir,z,first,type
COMMON DEF_widget,BUTTONS
COMMON SPLOT,stoks_in,stoks_out,xrng,plot_ind,plot_items,target,sent_ind,atm_ind,ps
COMMON OPTION,wx,thresh,PA,PA_null,PA_rot,Q_ISM,U_ISM,err,corr,Q_ism_old,U_ism_old
...
NN=numlines('d:\standards\sun_spectra\spectra.txt')
;sun=fltarr(2,NN)
;openr,1,'d:\standards\sun_spectra\spectra.txt'
;readf,1,sun
;close,1
num=plot_ind
...
;max_spectra=max(spectra.data(0:F-1,T+1)/sent)
max_spectra=max(spectra.data(R-1,T+1)/sent )

order=RFIX(ALOG10(max_spectra))
 wavL=[4861,5500,6565]
extin=calc_ext(spectra.data(R,0),40)

plot,spectra.data(R,0),spectra.data(R-1,T+1)/10.^order/sent/extin , $
...
;oplot,sun(0,*)*10,sun(1,*)/5,color=150,thick=3  ;рисование спектра —олнца
...
if target eq 0 then for L=0,2 do oplot,[1,1]*(1+z)*wavL(L),[-0.02,1.2]*max_spectra/10.^order,linestyle=2
if err gt 0  then OPLOTERR, wave(0:S-1),stoks.value(0:S-1,RN(k-1),T)/10.^order,stoks.error(0:S-1,RN(k-1),T)/10.^order/co,psym=10
...
;******************************************************
pro robust_estimate_stoks,ORDER2=order2,ROT=rot
COMMON GRAPH,spectra,stoks,wave,dir,z,first,type
COMMON FILES,LOG_dir,LOGFILE,pro_dir,wdir,woll,head,output_file
COMMON SPLOT,stoks_in,stoks_out,xrng,plot_ind,plot_items,target,sent_ind,range_Stoks,range_Angle
COMMON OPTION,wx,thresh,PA,PA_null,PA_rot,Q_ISM,U_ISM,err,corr,Q_ism_old,U_ism_old
...
;анализ нулевого и спектрофотометрического стандарта
	  	...
	  	null(0,k)=0
	   	print,'k=',k,WHERE(FINITE(null(*,k), /NAN))

	  	null(*,k)=LOWESS(findgen(Nx),null(*,k),Nx/8,2,2)
	  ;
...
;робастные оценки параметров —токса в окне
		print,Npos,Nx
		for k=0,Npol-1 do begin
 		for j=1,Npos-2 do begin
 		for i=0,Ntarget-1 do begin
 		IF k EQ 3 and V(i) EQ 0 THEN GOTO,OUT2
		robomean,stoks_in(xpos(j)-wx/2:xpos(j)+wx/2-1,k,0:spectra.Nexp(i)-1,i),thresh,0.5,avg_val,rms_val
		stoks.value(j,k,i)=avg_val
		stoks.error(j,k,i)=rms_val
		OUT2:
		endfor & endfor & endfor
...
print,PA_rot
		if first eq 1 then PA_null=317.3 else PA_null=0
		if type eq ' new' then PA_null=219 ELSE PA_null=178.9
		print,PA_null

		for j=0,Ntarget-1 do begin
		;TETA=2*(spectra.PA(j)-PA_null+PA_rot)*!PI/180
		TETA=2*(spectra.PA(j)+PA_null)*!PI/180
		Q=stoks.value(*,1,j)*cos(TETA)+stoks.value(*,2,j)*sin(TETA)
		U=stoks.value(*,1,j)*sin(TETA)-stoks.value(*,2,j)*cos(TETA)
		host=1 & sign=1
	;	if j eq 0 then host=2
	;	if j eq 0 then sign=-1
		stoks.value(*,1,j)=Q/host & stoks.value(*,2,j)=U/host*sign
		endfor

	;	endif
	    ;исправление межзвездной пол€ризации в спектре объекта
	    Q_ISM=-0.15 & U_ISM=1.02

	    stoks.value(*,1,0)=stoks.value(*,1,0)-Q_ISM/100.-Q_ism_old/100.
	    stoks.value(*,2,0)=stoks.value(*,2,0)-U_ISM/100.-U_ism_old/100.
	    Q_ism_old=Q_ism   &  U_ism_old=U_ism
        ;вычисление степени пол€ризации
		stoks.value(*,4,*)=sqrt(stoks.value(*,1,*)^2+stoks.value(*,2,*)^2)
		stoks.error(*,4,*)=sqrt(stoks.error(*,1,*)^2+stoks.error(*,2,*)^2)
		;поправка за нулевое смещение
		;stoks.value(*,4,*)=SQRT(stoks.value(*,4,*)-1.41*stoks.error(*,4,*))
		stoks.value(*,0,*)=stoks.value(*,0,*)*stoks.value(*,4,*)
		stoks.error(*,0,*)=stoks.value(*,0,*)*stoks.error(*,4,*)
		;вычисление угла плоскости пол€ризации
		FI=2*PA_rot*!PI/180
			;убирание неопределенности угла
		Q_tmp=stoks.value(*,1,0)*cos(FI)+stoks.value(*,2,0)*sin(FI)
		U_tmp=stoks.value(*,1,0)*sin(FI)-stoks.value(*,2,0)*cos(FI)
		stoks.value(*,5,*)=calc_atan(stoks.value(*,1,*),stoks.value(*,2,*))/2
		stoks.value(*,5,0)=calc_atan(Q_tmp,U_tmp)/2
...
;RF=WHERE(stoks.value(*,5,0) lt 25, ind) & if ind gt 0 then stoks.value(RF,5,0)=stoks.value(RF,5,0)+90
	;RF=WHERE(stoks.value(*,5,0) gt 90, ind) & if ind gt 0 then stoks.value(RF,5,0)=stoks.value(RF,5,0)-0
		;stoks.value(*,5,*)=atan(stoks.value(*,2,*)/stoks.value(*,1,*))/2*180/!PI
		for k=0,2 do begin
;RF=WHERE(stoks.value(*,5,k) gt 90, ind) & if ind gt 0 then stoks.value(RF,5,k)=stoks.value(RF,5,k)-90
;RF=WHERE(stoks.value(*,5,k) lt 90, ind) & if ind gt 0 then stoks.value(RF,5,k)=stoks.value(RF,5,k)+90

			endfor
;stoks.value(*,5,0)=stoks.value(*,5,0)-90
...
form_str=['','(F5.2)','(F5.2)','(F5.2)','(F5.2)','(F7.1)']
		...
		Wc=5500   & dW=500  & Rc=where(wave gt (Wc-dW)*(1+z) AND wave lt (Wc+dW)*(1+z))
		for j=0,Ntarget-1 do begin
...
Pro avg_Stoks_analyz_event, event
COMMON W_base,base_plot
COMMON DEF_widget,BUTTONS
COMMON GRAPH,spectra,stoks,wave,dir,z,first,type
COMMON FILES,LOG_dir,LOGFILE,pro_dir,wdir,woll,head,output_file
...
'LOG_input':BEGIN
			...
			type=''
			hf=headfits(wdir+'flat.fts')
			if sxpar(hf,'NAXIS3') eq 4 then begin
			if slope_flat(wdir) gt 0 then type=' new' ELSE type=' old'

			ENDIF
			WIDGET_CONTROL, BUTTONS(3), SET_VALUE='ANALYZER '+ANALYZER+type
			;установка начальных значений параметров
			Q_ism_old=0  & U_ism_old=0

			if   FILE_TEST(wdir+ 'stoks.fit') eq 1 then begin
			stoks_in=readfits(wdir+'stoks.fit',head)
			Nx=sxpar(head,'NAXIS1')
			lambda=findgen(Nx)*sxpar(head,'CDELT1')+sxpar(head,'CRVAL1')
			xrng(0)=min(lambda) & xrng(1)=max(lambda)
			WIDGET_CONTROL, BUTTONS(4), SET_VALUE=string(min(lambda),format='(I5)')+$
				'-'+string(max(lambda),format='(I5)')+' AA   '+$
			...
			;spectral coverage
			WIDGET_CONTROL, BUTTONS(14), SET_VALUE=string(xrng(0),format='(I5)')
			WIDGET_CONTROL, BUTTONS(15), SET_VALUE=string(xrng(1),format='(I5)')
			;чтение красного смещени€
			z=FLOAT(sxpar(read_table(LOGFILE),'redshift'))
			WIDGET_CONTROL, BUTTONS(16), SET_VALUE=string(z,format='(F10.7)')
			if target eq 0 and corr eq 1  then $
        	robust_estimate_stoks,/order2,/rot ELSE  robust_estimate_stoks,/rot
            multiplot
...
'SAVE':		BEGIN
			...
			;      avg_U            err_U
			stoks.value(R(k),2,0),stoks.error(R(k),2,0)],format='(I4,E12.3,4F7.2)')
			if V ne 0 then out_string=out_string+$
			string([stoks.value(R(k),3,0),stoks.error(R(k),3,0)],format='(2F7.2)')
			out_string=out_string+string([stoks.value(R(k),4,0),stoks.error(R(k),4,0)],format='(2F7.2)')
			out_string=out_string+string([stoks.value(R(k),5,0),stoks.error(R(k),5,0)],format='(F8.1,F8.1)')
			printf,1,out_string
			endfor
			close,1
			endif
...
PRO Avg_Stoks_analyz, GROUP=GROUP
COMMON W_base,base_plot
COMMON GRAPH,spectra,stoks,wave,dir,z,first,type
COMMON DEF_widget,BUTTONS
COMMON FILES,LOG_dir,LOGFILE,pro_dir,wdir,woll,head,output_file
...
draw_plot = WIDGET_DRAW(base_plot , XSIZE=800, YSIZE=800,/FRAME);vafan-w
;draw_plot = WIDGET_DRAW(base_plot , XSIZE=700, YSIZE=1100,/FRAME); vafan;, $
	;/MOTION_EVENTS,  /BUTTON_EVENTS ,$		;generate LOTS of events
	;UVALUE = 'DRAW_EVENT', 	RETAIN = 2)

base_par=WIDGET_BASE(base_plot,/col,xsize=210,ysize=10)
;set PATH
BUT_PATH=WIDGET_BUTTON(base_par,value='SET PATH',uvalue='set_PATH',ysize=20)
text_PATH=WIDGET_TEXT(BASE_par,value=log_dir,XSIZE=25,/align_center,/frame,ysize=1)
BUT_LOG=WIDGET_BUTTON(BASE_par,VALUE='OPEN LOGFILE',uvalue='LOG_input',ysize=20)
text_LOG=WIDGET_TEXT(base_par, XSIZE=25,/align_center)
;INFO ANALYZER
info_POL=WIDGET_LABEL(base_par,value='   ',xsize=160,/align_center,/frame,ysize=20)
INFO=WIDGET_LABEL(base_par,value='INIT SPECTRAL RANGE:',xsize=170,/align_center,ysize=15)
info_WAVE=WIDGET_LABEL(base_par,value='   ',xsize=160,/align_center,/frame)
INFO=WIDGET_LABEL(base_par,value='OUTPUT SPECTRAL RANGE:',xsize=170,/align_center,ysize=15)
;new spectral coverage
base_wave=WIDGET_BASE(base_par,/row,xsize=170)
INFO=WIDGET_LABEL(base_wave,value='OUTPUT',/align_center)
beg_wave=WIDGET_TEXT(base_wave,VALUE=string(xrng(0),format='(I4)'),uvalue='beg_wave',/edit,/align_center,xsize=4,/ALL_EVENT)
INFO=WIDGET_LABEL(base_wave,value='- ',/align_center)
end_wave=WIDGET_TEXT(base_wave,VALUE=string(xrng(1),format='(I4)'),uvalue='end_wave',/edit,/align_center,xsize=4,/ALL_EVENT)
INFO=WIDGET_LABEL(base_wave,value='AA',/align_center)

;
INFO=WIDGET_LABEL(base_par,value='OUTPUT TARGET',/align_center,/frame,xsize=160,ysize=15)
bobo=widget_base(base_par,xsize=260,/row)
target_but = CW_BGROUP(base_par,['obj','zero','star'] , /EXCLUSIVE,uvalue='target', set_value=target,/row,ysize=25 )
INFO=WIDGET_LABEL(base_par,value='OPTIONS OF THE OUTPUT PLOT',/align_center,/frame,xsize=260,ysize=15)
;
base_wave=WIDGET_BASE(base_par,/row,xsize=170,ysize=20)
INFO=WIDGET_LABEL(base_wave,value='window of robust estimate, px',/align_center,ysize=15)
...
threshold= WIDGET_TEXT(base_wave,VALUE=string(thresh,format='(I2)'),uvalue='thresh',/edit,/align_center,xsize=2,/ALL_EVENT)
;
PA_rot=0
base_wave=WIDGET_BASE(base_par,/row,xsize=170,ysize=20)
INFO=WIDGET_LABEL(base_wave,value='rotation plane of polarization',/align_center)
corr_angle= WIDGET_TEXT(base_wave,VALUE=string(PA_rot,format='(I3)'),uvalue='PA_rot',/edit,/align_center,xsize=3,/ALL_EVENT)
;

base_wave=WIDGET_BASE(base_par,/row,xsize=170,ysize=20)
INFO=WIDGET_LABEL(base_wave,value='ISM(%):  Q=',/align_center)
QISM=WIDGET_TEXT(base_wave,VALUE=string(Q_ISM,format='(F5.2)'),uvalue='Q_ISM',/edit,/align_center,xsize=5,/ALL_EVENT)

INFO=WIDGET_LABEL(base_wave,value='  U=',/align_center)
UISM=WIDGET_TEXT(base_wave,VALUE=string(U_ISM,format='(F5.2)'),uvalue='U_ISM',/edit,/align_center,xsize=5,/ALL_EVENT)
base_z=WIDGET_BASE(base_par,/row,xsize=170,ysize=20)
INFO=WIDGET_LABEL(base_z,value='redshift of object',/align_center)
z=0
zlab=WIDGET_TEXT(base_z,VALUE=string(z,format='(F10.7)'),uvalue='z',/edit,/align_center,xsize=10,/ALL_EVENT)

err=0
ERR_but = CW_BGROUP(base_par,'plot errors bar', /NONEXCLUSIVE,uvalue='err', set_value=err,ysize=25)
corr=0
CORR_but = CW_BGROUP(base_par,'correction 2nd order', /NONEXCLUSIVE,uvalue='corr', set_value=corr,ysize=25)
base_wave=WIDGET_BASE(base_par,/row,xsize=170)

INFO=WIDGET_LABEL(base_par,value='OUTPUT PLOTS',/align_center,/frame,xsize=160,ysize=15)

range_Stoks=2
base_wave=WIDGET_BASE(base_par,/row,xsize=170,ysize=15)
INFO=WIDGET_LABEL(base_wave,value='Output range of Stokes (%)',/align_center)
Stoks_lim=WIDGET_TEXT(base_wave,VALUE=string(range_Stoks,format='(I2)'),uvalue='Stoks_lim',/edit,/align_center,xsize=5,/ALL_EVENT)

range_angle=90
base_wave=WIDGET_BASE(base_par,/row,xsize=170)
INFO=WIDGET_LABEL(base_wave,value='0utput range of angle (deg)',/align_center)
Angle_lim=WIDGET_TEXT(base_wave,VALUE=string(range_angle,format='(I2)'),uvalue='Angle_lim',/edit,/align_center,xsize=5,/ALL_EVENT)
...
for k=0,5 do $
menu_plot = CW_BGROUP(base_par, plot_items(k), /NONEXCLUSIVE,uvalue='menu', IDS = button,set_value=plot_ind(k), /RETURN_NAME,ysize=25)
;values=WIDGET_LABEL(base_inform,value=' ',xsize=465)
button = WIDGET_BUTTON(base_par ,UVALUE = 'redraw',VALUE = 'redraw',ysize=20)
button = WIDGET_BUTTON(base_par ,UVALUE = 'SAVE',VALUE = 'SAVE RESULT',ysize=22)
base_out=WIDGET_BASE(base_par,/row,xsize=170,ysize=17)
INFO=WIDGET_LABEL(base_out,value='0utput file',/align_center)
OUT_NAME=WIDGET_TEXT(base_out,VALUE=' ',uvalue='output',/edit,/all,/align_center,xsize=24,/frame)
...
BUTTONS=[ draw_plot , text_PATH,text_LOG,info_POL,info_WAVE,box,corr_angle,QISM,UISM,CORR_but,$
		...
		threshold,Stoks_lim,Angle_lim,OUT_NAME,beg_wave,end_wave,zlab];,BUT_DEFAULT,BUT_LOAD,BUT_SAVE,Xbeg,wx,Ntra,DegTra,comp,label_pos,obj_pos,label_w,obj_w,radiusX,radiusY,$
		;    23      24            25                26        27       28
...
XMANAGER, "avg_Stoks_analyz", base_plot , GROUP_LEADER=GROUP, /NO_BLOCK
