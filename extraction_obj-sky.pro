...
FOR C=0,Ncub-1 do begin;Ncub-1 DO BEGIN
 ...
 xpos=findgen(Npos)*wx+wx/2+100 ;создаем реперы каждые wx пикселей
  for j=0,Nexp(c)-1 do begin
   for i=0,Npol-1 do begin
    for k=0,Npos-1 do begin
     Vy=total(cube(xpos(k)-wx/2:xpos(k)+wx/2-1,*,i,j,c),1)/wx
     ;определение положения максимума апертуры поперек щели
 ...
 for i=0,Npol-1 do begin
  ...
  ;plot,[0,Nx-1],[100,200],/nodata,xst=1,yst=1,charsize=2
   ...
   for j=0,Nexp(c)-1 do oplot,xpos,ypos(*,i,j), psym=2
  avg_tra=total(tra,size(tra,/n_dimensions))/Nexp(c)
;  oplot,x,avg_tra(*,i),thick=2
 ...
 for ky=0,Ny-1 do begin
  robomean,cube(Nx/2-5:Nx/2+5,ky,0,0,c),1,0.5,av ;определяем разрез спектра
  slice(ky)=av
 endfor
 cgdisplay,wid=12
 !p.multi=[0,1,1]
...
;			avs=total(spectra(kx,j,*))/N_elements(spectra(kx,j,*))
 ...
 for k=0,Nexp(c)-1 do begin
  robomean,spectra(x0:x0+M-1,0,k)/spectra(x0:x0+M-1,1,k),2,0.5,mean,rms ;находим среднее отношение двух поляризаций в интервале x0:x0+M
 ...
 for j=0,1 do begin
  ...
  oploterr,num,avg_ratio(j,*),rms_ratio(j,*)
  ;print, c, avg_ratio(j,*)
 endfor
ENDFOR
...
;расчет степени и угла поляризации
P=fltarr(Nx,3)
FI=P
for c=0,2 do begin
 Q(*,c)=Q(*,c);-Q_0
...
for ob=0,2 do begin
 ...
 cgplot, lam, F(*,ob),xrange=[lam_st,lam_fin], ytitle='ADU, counts', $
  title=names(ob)+' 16/12/2019   BTA+SCORPIO-2'
  ;cgoplot, [1,1]*6563*(1+z), [-1e5,1.2*max(F(*,ob))], color='red'
  ;cgoplot, [1,1]*4861*(1+z), [-1e5,1.2*max(F(*,ob))], color='red'
 cgplot, lam, V(*,ob), yrange=[median(V(*,ob))-5,median(V(*,ob))+5], xrange=[lam_st,lam_fin], ytitle='V, %', psym=10
