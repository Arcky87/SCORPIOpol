...
cgplot, lam(R),avg_spectra(R,2,stnum)/avg_spectra(R,3,stnum), yrange=[median(avg_spectra(R,2,stnum)/avg_spectra(R,3,stnum))-0.2,median(avg_spectra(R,2,stnum)/avg_spectra(R,3,stnum))+0.2], $
 ...
 ; Du=lowess(indgen(ind),avg_spectra(R,2,stnum)/avg_spectra(R,3,stnum),ind/6,1,1)
  Dv2=lowess(indgen(ind),avg_spectra(R,2,stnum)/avg_spectra(R,3,stnum),ind/6,1,1)
  cgoplot,lam(R),Dv2,color='gold',thick=3
;writefits, dir+'Dq.fts',dq,h
writefits, dir+'Dv1.fts',dv1,h
;print,'Dq.fts is stored! ', dir
print,'Dv1.fts is stored! ', dir
;writefits, dir+'Du.fts',du,h
writefits, dir+'Dv2.fts',dv2,h
;print,'Du.fts is stored! ', dir
print,'Dv2.fts is stored! ', dir
;stop
;Stokes plots
tot_spec=fltarr(ind,3)
;q=fltarr(ind,max(numexp),3) & u=fltarr(ind,max(numexp),3)
v=fltarr(ind,max(numexp),3)
...
; tot_spec(*,ob)=avg_spectra(R,0,ob)+avg_spectra(R,1,ob)*Dq+avg_spectra(R,2,ob)+avg_spectra(R,3,ob)*Du
 tot_spec(*,ob)=avg_spectra(R,0,ob)+avg_spectra(R,1,ob)*Dv1+avg_spectra(R,2,ob)+avg_spectra(R,3,ob)*Dv2
...
; d_Qres=fltarr(Npos) & d_Ures=fltarr(Npos)
  ...
  for k=0,Npos-1 do begin
   ...
   ; Ures(k)=mean & d_Ures(k)=rms;/3
    robomean,V(xpos(k)-w/2:xpos(k)+w/2-1,0:numexp(ob)-1,ob),1,0.5,mean,rms
    Vres(k)=mean & d_Vres(k)=rms;/3
   endif else begin
   ; robomean,Q(k,0:numexp(ob)-1,ob),1,0.5,mean,rms
   ; Qres(k)=mean & d_Qres(k)=rms;/3
   ; robomean,U(k,0:numexp(ob)-1,ob),1,0.5,mean,rms
   ; Ures(k)=mean & d_Ures(k)=rms;/3
    robomean,V(k,0:numexp(ob)-1,ob),1,0.5,mean,rms
    Vres(k)=mean & d_Vres(k)=rms;/3
   endelse
  endfor
...
;V panel
 cgplot, xpos(0:Npos-1)*cdel+lam_st, vres*100, yrange=[median(vres*100)-14.5,median(vres*100)+14.5], xrange=[lam_st,lam_fin], ytitle='V, %', psym=10, $
  err_ylow=d_Vres*100, err_yhigh=d_Vres*100, err_width=0, font=2, pos=[0.11,0.09+0.172*3,0.95,0.09+0.172*4], xtickformat="(A1)", charsize=chr, yminor=4
   ...
   for ll=0,N_elements(narlines)-1 do cgoplot, [1,1]*narlines(ll)*(1+z), [-10,10], linestyle=2
    if plot_atm eq 1 then begin
     mi=median(vres*100)-2.45 & ma=median(vres*100)+2.45
     for ia=0,N_elements(atm(0,*))-1 do begin
      vel=atm(*,ia)
      cgPolygon, [vel(0), vel(1),  vel(1),  vel(0), vel(0)], [mi,mi,ma,ma,mi], $
        ...
        cgoplot, xpos(0:Npos-1)*cdel+lam_st, vres*100, psym=10,err_ylow=d_Vres*100, err_yhigh=d_Vres*100, err_width=0
         cgtext,lam_fin-0.052*(lam_fin-lam_st), (median(vres*100)+1.5), '(2)', font=2, charsize=1.1
    robomean,vres*100,3,0.5,av,rms
    print, 'Mean v = ', av,'+-', rms, '%'
...
file=dir+objdir+'.txt'
 if ob eq 0 then begin
  ...
  ; printf, 4, xpos(k)*cdel+lam_st, qres(k)*100, d_qres(k)*100, ures(k)*100, d_ures(k)*100, p(k)*100, dp(k)*100, phi(k), dphi(k)
   printf, 4, xpos(k)*cdel+lam_st, vres(k)*100, d_vres(k)*100
  endfor
 close, 4
 endif
...
print, '*****OBJ specpol was written to '+dir+objdir+'.txt', ' (wavelength,V,dV)'
