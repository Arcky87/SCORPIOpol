...
ima=readfits(file_in,/silent)/scale
	...
	map(*,*,0)=ima(x_o-w:x_o+w-1,y_o-w:y_o+w-1) ; обыкновенный луч
	yfit=MPFIT2DPEAK(map(*,*,0), A, /TILT,/moffat )
	x_o=x_o+dx & y_o=y_o+dy
	map(*,*,0)=ima(x_o-w:x_o+w-1,y_o-w:y_o+w-1) ; обыкновенный луч поправка центра
	map(*,*,1)=ima(x_e-w:x_e+w-1,y_e-w:y_e+w-1) ; необыкновенный луч
	yfit=MPFIT2DPEAK(map(*,*,1), A, /TILT,/moffat )
	x_e=x_e+dx & y_e=y_e+dy
	map(*,*,1)=ima(x_e-w:x_e+w-1,y_e-w:y_e+w-1) ; необыкновенный луч поправка центра
for i=0,1 do begin
