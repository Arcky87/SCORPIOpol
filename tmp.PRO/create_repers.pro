...
pro create_repers,neon,tra,XPOS=pos,YPOS=ypos,xrep,yrep,TRESH=tresh,EPS=eps,PLOT=plot,WIN=win,TITL=titl
...
window,win,xsize=Nx/2,ysize=Ny,xpos=0,ypos=(Ny+40)*win,retain=2,title='create array of repers'+TITL
tv,255-bytscl(ALOG10(congrid(neon,Nx/2,Ny)),1,3)
...
xpk=find_peaks(vector,tresh=tresh )& Npk(k)=N_elements(xpk)
;точное определение положения пика
for j=0,Npk(k)-1 do begin
P=goodpoly(x(xpk(j)-wx:xpk(j)+wx),vector(xpk(j)-wx:xpk(j)+wx),2,3,fit)
xpk(j)=-P(1)/P(2)/2
endfor
...
f=goodpoly(yrep(*,k),xrep(*,k),1,3,Xfit)
err=stdev(xrep(*,k)-Xfit)
if err lt 0.5 then begin
if keyword_set(plot) then oplot,xrep(*,k),yrep(*,k),color=0,psym=6
index(k)=1
endif
endfor
