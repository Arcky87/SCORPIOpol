...
x_min=min([x1,x2])/sc & x_max=max([x1,x2])/sc
y_min=min([y1,y2])/sc & y_max=max([y1,y2])/sc
...
j=50./level
w=20
if x_2/sc gt w and x_2/sc lt Nx-1-w and $
	y_2/sc gt w and y_2/sc lt Ny-1-w then  begin
micromap=congrid(frame(x_2/sc-w:x_2/sc+w,y_2/sc-w:y_2/sc+w),100,100)
...
'DRAW_WIN_EVENT': BEGIN
			...
			if sw eq 1 then BOX
         	...
         	WIDGET_CONTROL,pixel_label, $
				   SET_VALUE='Pixel' + STRCOMPRESS('('+STRING(FIX(event.x/sc))+','+$
				   STRING(FIX(event.y/sc))+')')

			 if d_lambda gt 0 then $
				WIDGET_CONTROL,wave_label, $
				   SET_VALUE='wavelength' + STRCOMPRESS(STRING(FIX(event.x/sc*d_lambda+lambda_0))+'A') $
...
base_view = WIDGET_BASE(TITLE = 'VIEW '+file,xsize=y_size+125,ysize=y_size+30,/ROW,xoffset=520,yoffset=400)
...
XMANAGER, "viewZIP", base_view, GROUP_LEADER=GROUP, /NO_BLOCK
END
...
view_desktop,'h:\WOLLASTON-2.lib\S12810708.fts'
end